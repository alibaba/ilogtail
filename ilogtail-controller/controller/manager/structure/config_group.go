...
func (cg *ConfigGroup) AddConfig(newConfig *Config) error {
	id, flag := tool.FindPre(cg.Configs, newConfig.Name)
	if id >= MaxConfigNum {
	...
	//	structure.GetMyStore().sendMessage("M", "CG", cg.Name)
	return nil
}

// Delete a config from a group.
func (cg *ConfigGroup) DelConfig(configName string) error {
	id, flag := tool.FindPre(cg.Configs, configName)
...
func (cg *ConfigGroup) ChangeConfigName(configName string, newName string) error {
	...
	id1, flag1 := tool.FindPre(cg.Configs, configName)
	...
	id2, flag2 := tool.FindPre(cg.Configs, newName)
	if flag2 == true {
...
func AddConfigGroup(newConfigGroup *ConfigGroup) error {
	...
	//	structure.GetMyStore().sendMessage("U", "CG", newConfigGroup.Name)
	return nil
}
...
func DelConfigGroup(configGroupName string) error {
	...
	if ok {
		...
		//		structure.GetMyStore().sendMessage("D", "CG", configGroupName)
		return nil
	}
