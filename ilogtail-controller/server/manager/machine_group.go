...
type MachineGroup struct {
	...
	Machines    []string `json:"machine_uids"`
...
func (mg *MachineGroup) AddMachine(newMachine *Machine) error {
	id, flag := findPre(mg.Machines, newMachine.Uid)
	if id > MaxMachineNum {
	...
	mg.Machines = append(mg.Machines[:id], append([]string{newMachine.Uid}, mg.Machines[id:]...)...)
	return nil
}

// Add a config to a group.
func (mg *MachineGroup) AddConfig(newConfig *Config) error {
	id, flag := findPre(mg.Configs, newConfig.Name)
	if id > MaxConfigNum {
		return errors.New("Exceed the maximum number limit, add failed.")
	}
	if flag == true {
...
func (mg *MachineGroup) DelMachine(machineUid string) error {
	id, flag := findPre(mg.Machines, machineUid)
...
func (mg *MachineGroup) DelConfig(configName string) error {
	id, flag := findPre(mg.Configs, configName)
...
func (mg *MachineGroup) ChangeMachineName(machineName string, newName string) error {
	...
	id1, flag1 := findPre(mg.Machines, machineName)
	...
	id2, flag2 := findPre(mg.Machines, newName)
	if flag2 == true {
