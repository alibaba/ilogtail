...
// this tool is for generate files integrating external plugins
// usage: builder -root <project-root-dir> -config <plugins-config-file.yaml> -modfile <custom-go.mod-file>

var configFile = flag.String("config", "external_plugins.yml", "file path or url including external plugins config")
...
func main() {
	...
	conf, err := loadPluginConfig(*configFile)
	if err != nil {
		fmt.Println("fail to load config from file:", *configFile)
		os.Exit(1)
	}
	if conf.Plugins.IsEmpty() {
		fmt.Println("empty external plugins config file, nothing to do")
		return
	}
...
func applyGitConfigs(ctx *buildContext) error {
	...
	for k, v := range ctx.Config.Project.GitConfigs {
		cmd := exec.Command(ctx.GitExe, "config", k, v)
