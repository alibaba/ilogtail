...
func (s *logConfigTestSuite) TestFile(c *check.C) {
	info := MockDockerInfoDetail("containerName", []string{
		"aliyun_logs_catalina=C:\\usr\\local\\tomcat\\logs\\catalina.*.log",
	})
	...
	c.Assert(config.LogtailConfig.LogtailConfig["logPath"].(string), check.Equals, "c:\\usr\\local\\tomcat\\logs\\")
	...
	c.Assert(config.LogtailConfig.LogtailConfig["dockerIncludeEnv"].(map[string]string)["aliyun_logs_catalina"],
		check.Equals, "C:\\usr\\local\\tomcat\\logs\\catalina.*.log")
}
