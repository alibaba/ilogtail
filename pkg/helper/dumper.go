...
func (d *Dumper) Init() {
	_ = os.MkdirAll(path.Join(util.GetCurrentBinaryPath(), "../data/", "dump"), 0750)
	d.input = make(chan *DumpData, 10)
	d.stop = make(chan struct{})
	files, err := GetFileListByPrefix(path.Join(util.GetCurrentBinaryPath(), "../data/", "dump"), d.prefix, true, 0)
	if err != nil {
...
func (d *Dumper) doDumpFile() {
	...
	cutFile := func() (f *os.File, err error) {
		nFile := path.Join(path.Join(util.GetCurrentBinaryPath(), "../data/", "dump"), fileName+"_"+time.Now().Format("2006-01-02_15"))
