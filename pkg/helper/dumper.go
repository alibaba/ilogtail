...
func (d *Dumper) Init() {
	...
	files, err := GetFileListByPrefix(path.Join(config.LoongcollectorGlobalConfig.LoongcollectorConfDir, "dump"), d.prefix, true, 0)
	if err != nil {
...
func (d *Dumper) doDumpFile() {
	...
	cutFile := func() (f *os.File, err error) {
		nFile := path.Join(path.Join(config.LoongcollectorGlobalConfig.LoongcollectorConfDir, "dump"), fileName+"_"+time.Now().Format("2006-01-02_15"))
