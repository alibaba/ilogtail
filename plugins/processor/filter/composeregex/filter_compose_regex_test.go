...
func (s *processorTestSuite) SetUpTest(c *check.C) {
	s.processor = ilogtail.Processors["processor_filter_compose_regex"]()
...
func (s *processorTestSuite) TestInitError(c *check.C) {
	processor, _ := s.processor.(*ProcessorComposeRegexFilter)
...
func (s *processorTestSuite) TestDefault(c *check.C) {
	{
		...
		processor, _ := s.processor.(*ProcessorComposeRegexFilter)
		processor.Fields = map[string]string{"aaa":"111","key1":"2222"}
