...
EXTERNAL_PLUGINS_CONFIG_FILE=${8:-${EXTERNAL_PLUGINS_CONFIG_FILE:-external_plugins.yml}}
...
EOF
  ...
  if [ $CATEGORY = "plugin" ]; then
    echo "mkdir -p core/build && cd core/build && cmake -DCMAKE_BUILD_TYPE=Release -DLOGTAIL_VERSION=${VERSION} .. && cd plugin && make -s PluginAdapter && cd ../../.. && ./scripts/plugin_build.sh mod c-shared ${OUT_DIR} ${EXTERNAL_PLUGINS_CONFIG_FILE} ${GO_MOD_FILE}" >> $BUILD_SCRIPT_FILE;
