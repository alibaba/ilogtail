...
func (wrapper *MetricWrapperV1) Run(control *pipeline.AsyncControl) {
	...
	if wrapper.Config.GlobalConfig.GoInputToNativeProcessor {
		...
		wrapper.eventCached = make([]*protocol.LogEvent, 0, wrapper.MaxCachedSize+1)
		wrapper.tagCached = make([]map[string]string, 0, wrapper.MaxCachedSize+1)
		wrapper.ctxCached = make([]map[string]interface{}, 0, wrapper.MaxCachedSize+1)
