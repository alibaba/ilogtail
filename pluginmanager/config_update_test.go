...
func (s *configUpdateTestSuite) TestConfigUpdate() {
	...
	checkFlusher, ok := config.PluginRunner.(*pluginv1Runner).FlusherPlugins[0].Flusher.(*checker.FlusherChecker)
	s.True(ok)
	...
	s.Equal(200000, LogtailConfig[noblockUpdateConfigName].PluginRunner.(*pluginv1Runner).FlusherPlugins[0].Flusher.(*checker.FlusherChecker).GetLogCount())
}
...
func (s *configUpdateTestSuite) TestConfigUpdateMany() {
	...
	checkFlusher, ok = LogtailConfig[noblockUpdateConfigName].PluginRunner.(*pluginv1Runner).FlusherPlugins[0].Flusher.(*checker.FlusherChecker)
	s.True(ok)
	time.Sleep(time.Second * time.Duration(3))
...
func (s *configUpdateTestSuite) TestConfigUpdateName() {
...

	{
		...
		checkFlusher, ok := LogtailConfig[updateConfigName+"_"].PluginRunner.(*pluginv1Runner).FlusherPlugins[0].Flusher.(*checker.FlusherChecker)
		s.True(ok)
