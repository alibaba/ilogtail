...
func (i *InfluxdbSubscriber) runQueryRecordsTask() {
	for {
		select {
		case <-i.stopCh:
			close(i.channel)
		...
		case <-time.After(time.Duration(i.QueryIntervalMs) * time.Millisecond):
			...
			logGroup, maxTimestamp := i.queryRecords2LogGroup(resp.Results)
...
func (i *InfluxdbSubscriber) queryRecords2LogGroup(results []client.Result) (logGroup *protocol.LogGroup, maxTimestamp int64) {
			...
			for _, values := range serie.Values {
				for _, field := range serie.Columns[1:] {
					for _, v := range values[1:] {
						...
						if t, _ := strconv.ParseInt(timestamp, 10, 64); t > maxTimestamp {
							maxTimestamp = t
						}
