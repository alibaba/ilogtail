...
CommonConfigProvider::SendHeartbeat(const ConfigServerAddress& configServerAddress) {
    ...
    heartBeatReq.set_agent_id(AppConfig::GetInstance()->GetInstanceId());
    ...
    for (unordered_map<string, int64_t>::iterator it = mServerYamlConfigVersionMap.begin();
...
google::protobuf::RepeatedPtrField<configserver::proto::ConfigDetail> CommonConfigProvider::FetchPipelineConfig(
    ...
    string requestID
        = sdk::Base64Enconde(AppConfig::GetInstance()->GetInstanceId().append("_").append(to_string(time(NULL))));
    fetchConfigReq.set_request_id(requestID);
    fetchConfigReq.set_agent_id(AppConfig::GetInstance()->GetInstanceId());
...
void CommonConfigProvider::UpdateRemoteConfig(
    ...
    static string serverConfigDirPath = mSourceDir.string();
