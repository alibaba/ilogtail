...
void AppConfig::LoadAddrConfig(const Json::Value& confJson) {
    ...
    // api prefix for configserver
    if (confJson.isMember("ilogtail_api_prefix") && confJson["ilogtail_api_prefix"].isString()) {
        mApiPrefix = TrimString(confJson["ilogtail_api_prefix"].asString());
    }
}
