...
class VectorBuilder {
   public:
    ...
    VectorBuilder(std::shared_ptr<memory::MemoryPool> pool = nullptr) {
        pool_ = pool != nullptr ? pool : memory::addDefaultLeafMemoryPool();
        vectorMaker_ = std::make_shared<facebook::velox::test::VectorMaker>(pool_.get());
    }
    ...
    BufferPtr makeIndicesInReverse(vector_size_t size) {
        return ::apsara::sls::spl::makeIndicesInReverse(size, pool());
    }
    ...
    std::shared_ptr<memory::MemoryPool> getPoolPtr() const {
        return pool_;
    }
    ...
    std::shared_ptr<memory::MemoryPool> pool_{nullptr};
    std::shared_ptr<velox::test::VectorMaker> vectorMaker_{nullptr};
};

using VectorBuilderPtr = std::shared_ptr<VectorBuilder>;
