...
void PollingDirFile::Start() {
    ClearCache();
    mGlobalConfigTotal = LoongCollectorMonitor::GetInstance()->GetGauge(METRIC_GLOBAL_CONFIG_TOTAL);
    mGlobalPollingDirCacheSizeTotal
        = LoongCollectorMonitor::GetInstance()->GetGauge(METRIC_GLOBAL_POLLING_DIR_CACHE_SIZE_TOTAL);
    mGlobalPollingFileCacheSizeTotal
        = LoongCollectorMonitor::GetInstance()->GetGauge(METRIC_GLOBAL_POLLING_FILE_CACHE_SIZE_TOTAL);
    mRuningFlag = true;
