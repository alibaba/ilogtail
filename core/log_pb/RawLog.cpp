...
RawLog*
RawLog::AddLogFull(uint32_t logTime, std::vector<std::string> keys, boost::match_results<const char*> subMathValues) {
...
uint32_t RawLog::AddKeyValue(const char* key, size_t key_len, const char* value, size_t value_len) {
    // sum total size
    uint32_t kv_size
        = sizeof(char) * (key_len + value_len) + uint32_size((uint32_t)key_len) + uint32_size((uint32_t)value_len) + 2;
...
bool RawLog::NextKeyValue(
    RawLog::iterator& iter, const char*& key, uint32_t& keyLen, const char*& value, uint32_t& valueLen) {
...
bool RawLog::UpdateKeyValue(
    RawLog::iterator& iter, const char* key, size_t key_len, const char* value, size_t value_len) {
...
bool RawLog::AppendKeyValue(const char* key, size_t kenLen, const char* value, size_t valueLen) {
