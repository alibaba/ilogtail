...
public:
    void SetUp() {
        ...
        pluginMetricManager = std::make_shared<PluginMetricManager>(mMetricsRecordRef->GetLabels());
    }
...
void PluginMetricManagerUnittest::TestInitPluginMetricManager() {
    ...
    APSARA_TEST_EQUAL(pluginMetricManager->mMetricsRecordRefsMap.size(), 0);
}
...
void PluginMetricManagerUnittest::TestGetOrCreateMetricsRecordRefPtr() {
    ...
    auto ptr_1 = pluginMetricManager->GetOrCreateMetricsRecordRefPtr(labels);
    APSARA_TEST_NOT_EQUAL(ptr_1, nullptr);
    APSARA_TEST_EQUAL(ptr_1->GetLabels()->size(), 7);
    APSARA_TEST_EQUAL(pluginMetricManager->mMetricsRecordRefsMap.size(), 1);
    APSARA_TEST_EQUAL(ptr_1.use_count(), 2);

    auto ptr_2 = pluginMetricManager->GetOrCreateMetricsRecordRefPtr(labels);
...
void PluginMetricManagerUnittest::TestReleaseMetricsRecordRefPtr() {
    ...
    auto ptr = pluginMetricManager->GetOrCreateMetricsRecordRefPtr(labels);
    APSARA_TEST_NOT_EQUAL(ptr, nullptr);
    APSARA_TEST_EQUAL(ptr->GetLabels()->size(), 7);
    APSARA_TEST_EQUAL(pluginMetricManager->mMetricsRecordRefsMap.size(), 1);

    pluginMetricManager->ReleaseMetricsRecordRefPtr(labels);
    APSARA_TEST_EQUAL(pluginMetricManager->mMetricsRecordRefsMap.size(), 0);

    auto ptr2 = pluginMetricManager->GetOrCreateMetricsRecordRefPtr(labels);
...
void PluginMetricManagerUnittest::TestRegisterSizeGauge() {
    GaugePtr sizeGauge = mMetricsRecordRef.GetOrCreateGauge("test_gauge");
