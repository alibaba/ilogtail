...
void PipelineUnittest::OnSuccessfulInit() const {
    ...
    APSARA_TEST_EQUAL(0U, pipeline->mInProcessCntWhenStop.load());
...
void PipelineUnittest::TestInProcessingCount() const {
    ...
    pipeline->mPluginID.store(0);
    pipeline->mInProcessCntWhenStop.store(0);
    ...
    APSARA_TEST_EQUAL(0, pipeline->mInProcessCntWhenStop.load());
    unique_ptr<ProcessQueueItem> item;
    APSARA_TEST_TRUE(processQueue->Pop(item));
    APSARA_TEST_EQUAL(1, pipeline->mInProcessCntWhenStop.load());
