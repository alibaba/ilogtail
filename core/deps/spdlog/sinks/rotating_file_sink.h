...
namespace sinks {
    ...
    public:
        rotating_file_sink(filename_t base_filename, std::size_t max_size, std::size_t max_files)
            ...
            // baoze.yyh: rotate at beginning.
            auto fn = calc_filename(base_filename_, 0);
            auto existing = details::file_helper::file_exists(fn);
            file_helper_.open(fn);
            if (existing) {
                rotate_();
            }
...
template <typename Factory = default_factory>
inline std::shared_ptr<logger>
rotating_logger_st(const std::string& logger_name, const filename_t& filename, size_t max_file_size, size_t max_files) {
